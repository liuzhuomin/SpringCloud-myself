...
//        AND p.policyGroup IS NOT NULL
        StringBuffer hql = new StringBuffer(
                "SELECT new PolicyDto(p.id as id,p.shortTitle as title ) FROM Policy AS p WHERE status='P' AND p.policyGroup IS NOT NULL AND id not in (SELECT  policy.id AS id FROM PolicyTemplate WHERE policy IS NOT NULL)");
        if (ignoreId == null) {
...
//    }
    ...
    public boolean checkNotCreate(Long policyId) {
        List<Long> listObjByHql = dao.pageObjectByHql(
                "SELECT id FROM PolicyTemplate WHERE policy.id = ?0", Long.class, policyId);
