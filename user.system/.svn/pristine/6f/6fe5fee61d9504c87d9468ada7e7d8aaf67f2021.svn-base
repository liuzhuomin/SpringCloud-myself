...
public class PolicyServiceImpl extends BaseServiceImpl<Policy> implements PolicyService {
    ...
    public List<PolicyGroupDto> listAllGroup() {
        return dao.pageObjectByHql("select new PolicyGroupDto(id,title) from PolicyGroup", PolicyGroupDto.class);
    }
    ...
    private void setPolicyParagraph(PolicyDto policyDto) {
        ...
        String query = dao.getJdbcTemplate().query("select content from zhjf_policy_paragraph where id =?", new ResultSetExtractor<String>() {
            @Override
